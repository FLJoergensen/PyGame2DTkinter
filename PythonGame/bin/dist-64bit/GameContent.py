...
class Path:
    def __init__(self,Map,StartKoordinat,EndKoordinat):
        self.Map=Map
        self.start=Map.getV(StartKoordinat[0],StartKoordinat[1])
        self.end=Map.getV(EndKoordinat[0],EndKoordinat[1])
        pass
    def surch(self):
        #ToDo
        dic={}
        for item in self.Map.getAll():
            dic[item]=None
        return []
